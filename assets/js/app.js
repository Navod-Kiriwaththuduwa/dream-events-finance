(() => {
  ...
  function expenseTable(items,approvalsOnly=false){ return `<section class="panel table-panel"><table><thead><tr><th>Expense</th><th>Event</th><th>Category</th><th>Paid From</th><th>Amount</th><th>Status</th>${approvalsOnly?'<th>Action</th>':''}</tr></thead><tbody>${items.length?items.map(x=>`<tr><td><b>${escapeHtml(x.description||x.expenseId)}</b><small>${escapeHtml(x.expenseId)} · ${escapeHtml(x.date)}${x.budgetLineId?' · Budget linked':''}${x.attachmentUrl?` · <a href="${escapeHtml(x.attachmentUrl)}" target="_blank" rel="noopener">View attachment</a>`:''}</small></td><td>${escapeHtml(x.eventId||'Business')}</td><td>${escapeHtml(x.category)}${x.subCategory?`<small>${escapeHtml(x.subCategory)}</small>`:''}</td><td>${escapeHtml(String(x.paidFrom||'').replaceAll('_',' '))}</td><td class="money">${money(x.amount)}</td><td><span class="${statusClass(x.status)}">${escapeHtml(x.status)}</span></td>${approvalsOnly?`<td><div class="row-actions"><button class="btn btn-xs btn-success" data-approve="${x.expenseId}">Approve</button><button class="btn btn-xs btn-danger" data-reject="${x.expenseId}">Reject</button></div></td>`:''}</tr>`).join(''):`<tr><td colspan="${approvalsOnly?7:6}" class="empty">No expenses found.</td></tr>`}</tbody></table></section>`; }
  ...
  async function expenseAttachmentPayload(file){
    if(!file || !file.name || !file.size) return null;
    const maxBytes=5*1024*1024;
    if(file.size>maxBytes) throw new Error('Receipt / bill must be 5 MB or smaller.');
    const mime=String(file.type||'').toLowerCase();
    if(!(mime==='application/pdf' || mime.startsWith('image/'))) throw new Error('Receipt / bill must be an image or PDF file.');
    ...
    if(comma<0) throw new Error('Unable to prepare the selected attachment.');
    return {name:file.name,mimeType:mime||'application/octet-stream',base64:dataUrl.slice(comma+1)};
  }
  ...
  async function expenseModal(presetEventId=''){
    ...
    $('#expenseForm').addEventListener('submit',async e=>{
      e.preventDefault();
      const form=e.target, submitBtn=$('#expenseSubmitBtn');
      const fd=new FormData(form), file=fd.get('attachment');
      const data=Object.fromEntries(fd); delete data.attachment;
      submitBtn.disabled=true; submitBtn.textContent=file&&file.size?'Uploading…':'Saving…';
      try{
        data.attachment=await expenseAttachmentPayload(file);
        await API.request('createExpense',data);
        closeModal();
        toast(session.user.role==='FINANCE_HEAD'?'Expense approved and recorded.':'Expense submitted for approval.');
        if(state.route==='eventDetail')openEvent(state.currentEvent.eventId,'expenses');else navigate(state.route==='approvals'?'approvals':'expenses');
      }catch(err){
        toast(err.message,'error');
        submitBtn.disabled=false; submitBtn.textContent='Submit Expense';
      }
    });bindModalCloseButtons();
  }
