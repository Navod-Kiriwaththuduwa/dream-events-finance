const SHEET_SCHEMAS = {
  ...
  '17_EXPENSES':['Expense_ID','Event_ID','Expense_Scope','Main_Category','Sub_Category','Description','Supplier_ID','Amount','Payment_Method','Paid_From','Expense_Date','Attachment_URL','Approval_Status','Submitted_By','Submitted_At','Approved_By','Approved_At','Locked','Rejection_Reason','Notes','Created_At'],
...
function seedCounters_(){ const y=new Date().getFullYear(); if(getRows_('29_COUNTERS').length)return; ['EVENT','QUOTE','INVOICE','RECEIPT','EXPENSE','INCOME','CUSTOMER','SUPPLIER','INVENTORY','PAYABLE'].forEach(t=>appendRow_('29_COUNTERS',[t,y,0,nowIso_()])); }
