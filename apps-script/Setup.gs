const SHEET_SCHEMAS = {
  ...
  '12_INVOICE_LINES':['Invoice_Line_ID','Invoice_ID','Level','Main_Item','Sub_Item','Description','Qty','Unit_Price','Amount','Display_Order'],
  '13_PAYMENT_PLANS':['Payment_Plan_ID','Invoice_ID','Event_ID','Sequence','Milestone_Name','Percentage','Expected_Amount','Due_Date','Received_Amount','Balance','Status','Plan_Type'],
...
function seedCounters_(){ const y=new Date().getFullYear(); const existing=getRows_('29_COUNTERS'); ['EVENT','QUOTE','INVOICE','RECEIPT','PAYMENT','PAYMENT_PLAN','EXPENSE','INCOME','CUSTOMER','SUPPLIER','INVENTORY','PAYABLE','BUDGET','BUDGET_LINE'].forEach(t=>{if(!existing.some(r=>r.Type===t&&Number(r.Year)===y))appendRow_('29_COUNTERS',[t,y,0,nowIso_()]);}); }
